...
/**
 * This class is the UI representation of Level One within the CodeEsc
 ...
 * the actual level.
 */
public class CodeEscLevelOne extends JFrame {
    ...
    /**
     * Constructs a new CodeEscLevelOne instance, makes the CodeEsc menu invisible and itself visible,
     ...
     */
    public CodeEscLevelOne(CodeEscMenu ownerMenu) {
        // Set up the JFrame
        setTitle("CodeESC Level One");
        setSize(800, 800);
        ...
        ImageIcon tempBackgroundImage = new ImageIcon("levelOneBackground.png");
    ...
    private void initializeLevel() {
        ...
        bookButton.setBounds(73, 508, 190, 50);
        computerButton.setBounds(288, 337, 208, 232);
        safeButton.setBounds(540, 382, 180, 170);

        // Call a method to make these buttons invisible
        handleLevelInterface.makeButtonInvisible(bookButton);
        handleLevelInterface.makeButtonInvisible(computerButton);
        ...
        handleLevelOneLogic levelLogic = new handleLevelOneLogic(bookButton, computerButton, safeButton, this);

        // Construct a label to give starting hint for the Escape Room level
        JLabel hintLabel = new JLabel();
        hintLabel.setText("<html>The one with pages shall start the quest, <br>\n" +
                        "The secret password unlocks the next. <br>\n" +
                        "Type it in where the screen must glow, <br>\n" +
                        "The safe holds the key to let you go.</html>");
        hintLabel.setFont(new Font("Bauhaus 93", Font.BOLD, 22));
        hintLabel.setOpaque(true);
        hintLabel.setBackground(new Color(110,60,120,190));
        hintLabel.setForeground(Color.black);
        hintLabel.setBorder(BorderFactory.createLineBorder(new Color(20,20, 20), 2));
        hintLabel.setBounds(0, 50, 450, 180);
    ...
    public void finishLevel(int secondsPassed) {
        levelTimeQueries.insertTime(ownerMenu.getAccount().getCon(), ownerMenu.getAccount().getUsername(), 1, secondsPassed);
        System.out.println("Finished level.");
        setVisible(false);
        ownerMenu.setVisible(true);
        ownerMenu.getMenuLevel().getLevelOneBtn().updateTime();

        JOptionPane.showMessageDialog(this, "<html>You successfully completed Level One of CodeEsc!<br>Your time was: " + String.format("%02d:%02d", secondsPassed/60, secondsPassed%60) + "</html>");
    }
